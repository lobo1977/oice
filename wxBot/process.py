...
def z_encode(p):
    ...
    if p == None:                               #None->PHP中的NULL
        ...
        return "i:%d;" % p
    elif isinstance(p, unicode):                #unicode 字符串
        ret = 's:%d:"' % len(p)
        ret = ret.encode(php_python.CHARSET)
        ret = ret + p + '";'.encode(php_python.CHARSET)
    ...
    def run(self):
        ...
        try:  
            ...
            if len(firstbuf) < REQUEST_MIN_LEN:               #不够消息最小长度
                ...
                print '[ERROR] Less than minimum length:',firstbuf
                self._socket.close()
                return
            ...
            print '[INFO] message length:',totalLen
            reqMsg = firstbuf[firstComma+1:]
            print '[INFO] reqMsg:',reqMsg
            ...
            #print ('接收消息异常', e)
            print '[ERROR] getMessage error',str(e)
            self._socket.close()
            return
        ...
        print '[INFO] module:',modul,'func:',func,'parmas:',params
        ...
        if (modul not in pc_dict):   #预编译字典中没有此编译模块
            ...
            try:
                ...
                print '[ERROR] module not exist:',modul
                #print ('模块不存在:%s' % modul)
        ...
        try:
            ...
            print '[ERROR] function not exist:',func
            #print ('函数不存在:%s' % func)
        ...
        try: 
            ...
            print '[ERROR] call python error:',str(e)
