...
class WXBot:
    ...
    def proc_at_info(msg):
        ...
        if len(segs) > 1:
            for i in range(0, len(segs) - 1):
                segs[i] += u'\u2005'
                pm = re.search(u'@.*\u2005', segs[i])
                if pm:
                    pg = pm.group()
                    name = pg[1:-1]
                    string = segs[i].replace(pg, '')
    ...
    def proc_msg(self):
        ...
        while True:
            if self.status == 'wait4loginout':  #WxbotManage使用
                print '[DEBUG] wait4loginout', time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                return 
            check_time = time.time()
            try:
                [retcode, selector] = self.sync_check()
                if retcode == '1100':  # 从微信客户端上登出
                    print '[DEBUG] sync_check: 1100', time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                    break
                elif retcode == '1101':  # 从其它设备上登了网页微信
                    print '[DEBUG] sync_check: 1101', time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                    break
                elif retcode == '0':
                    if selector == '2':  # 有新消息
                        r = self.sync()
                    ...
                    print '[DEBUG] sync_check: ' + retcode, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                    time.sleep(10)
                self.schedule()
            except:
    ...
    def wait4login(self):
        ...
        tip=0, 等待用户确认登录,
               200: confirmed
        """
        LOGIN_TEMPLATE = 'https://login.wx.qq.com/cgi-bin/mmwebwx-bin/login?tip=%s&uuid=%s&_=%s'
        tip = 1
        ...
        while retry_time > 0:
            ...
            if code == SCANED:
                ...
                self.sync_host = 'webpush.' + self.base_host
                print '[INFO] base_uri:' + self.base_uri
                return code
            elif code == TIMEOUT:
    ...
    def login(self):
        print '[INFO] %s' % self.redirect_uri
    ...
    def sync_check(self):
        ...
        try:
            ...
            print '[ERROR] Except in sync_check'
            print format_exc()
            return [-1, -1]
